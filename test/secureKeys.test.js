...
test('secureKeys: writeTempKey creates file with correct content and permissions', async (t) => {
  ...
  try {
    ...
    t.equal(stats.mode & 0o777, 0o600, 'File permissions should be 600 initially')

    // Verify mocked sudo calls
    t.equal(execFileSyncStub.callCount, 2, 'Should call execFileSync twice (chown + chmod)')
...
test('secureKeys: writeTempKey handles sudo failure (chown)', async (t) => {
  ...
  try {
    ...
    t.ok(err.message.includes('Failed to set ownership/permissions via sudo'), 'Error message should indicate sudo failure')
    t.ok(err.message.includes('Permission denied'), 'Error message should include original error')
  }
...
test('secureKeys: writeTempKey handles sudo failure (chmod)', async (t) => {
  ...
  try {
    ...
    t.ok(err.message.includes('Failed to set ownership/permissions via sudo'), 'Error message should indicate sudo failure')
    t.ok(err.message.includes('Operation not permitted'), 'Error message should include original error')
  }
